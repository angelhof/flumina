...
At the moment, rates are given for processes in nodes, rather than for nodes.
  ...
  - [ ] Create a producer for each tag, and then given the configuration tree,
  ...
  - [ ] Implement a generic node source/producer, that receives/sends all the 
...
*** TODO Implement a complete root tree to setup tree function
...
previously to allow us to make this setup tree.
   ...
   - [ ] Make the root to setup tree complete, in the sense that it should

...
*** TODO Move the implementation source in ./src and the examples in ./examples

*** TODO Instead of sending merge requests from parent nodes, send them immediately from the input

*** TODO Make sure that the top nodes propagate heartbeats to children nodes
...
*** TODO Implement the buffer as a partial order that captures the dependencies

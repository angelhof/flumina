...
*** TODO Separate timestamp and node id from the message, as it is never needed for the computation

TODO: node.erl calls the preds of its children to call the split, 
      however, split predicates are specification predicates, and so we should keep somewhere
      in the tree, the spec predicate of each node.

TODO: Make sure that the split and merge predicates are correct. 
Should they be about implementation, or specification tags?
...
should be for the specification.
  ...
  - [ ] Splits/Merges predicates
  - [X] Update type definitions
  - [-] Update code in src
    + [X] node.erl
    + [X] producer.erl
    + [X] configuration.erl
    + [ ] optimizer_greedy.erl
...
*** TODO Timestamp based producers might drift behind one another 

Find a way to solve this problem. This problem also leads to high latency.
